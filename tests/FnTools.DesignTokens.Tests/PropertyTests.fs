module FnTools.DesignTokens.Tests.PropertyTests

open Expecto
open Hedgehog
open FnTools.DesignTokens
open FnTools.DesignTokens.Tests.Generators
    ...
    testList "Properties" [
        ...
        testCase "flatten yields one entry per child for flat files" <| fun () ->
            Property.checkBool <| property {
                ...
                let flat = FnTools.DesignTokens.Api.Primitives.flatten file |> List.ofSeq
            ...
            Property.checkBool <| property {
                ...
                let allFound =
                    ...
                    |> List.forall (fun (path, _) ->
                        match FnTools.DesignTokens.Api.Primitives.tryFind path file with
                        | Some _ -> true
                        | None -> false)
                return allFound
            }
