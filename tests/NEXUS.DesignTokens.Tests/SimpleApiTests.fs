module NEXUS.DesignTokens.Tests.SimpleApiTests
...
let allTests =
    testList "SimpleApi" [
        ...
        testCase "import: returns ResolvedToken seq for valid file" <| fun () ->
            match NEXUS.DesignTokens.Api.import V2025_10.colorBrandJson with
        ...
        testCase "import: invalid JSON yields ParseFailed" <| fun () ->
            match NEXUS.DesignTokens.Api.import "not json" with
            | Error errs ->
                let isParseFailed =
        ...
        testCase "import: alias resolved to terminal literal" <| fun () ->
            match NEXUS.DesignTokens.Api.import V2025_10.aliasJson with
            | Error es -> failtestf "import failed: %A" es
            | Ok seq ->
                let xs = seq |> List.ofSeq
        ...
        testCase "export: round-trips through resolved tokens" <| fun () ->
            ...
            | Error es -> failtestf "import failed: %A" es
            | Ok seq ->
                let json = NEXUS.DesignTokens.Api.export seq
        ...
        testCase "importWithResolver: applies default context" <| fun () ->
            match NEXUS.DesignTokens.Api.importWithResolver noLoad Map.empty Resolver.basicResolverJson with
