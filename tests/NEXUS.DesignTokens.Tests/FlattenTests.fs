module NEXUS.DesignTokens.Tests.FlattenTests
...
let allTests =
    testList "Flatten" [
        ...
        testCase "flatten yields all leaf paths" <| fun () ->
            ...
            | Ok file ->
                let xs = NEXUS.DesignTokens.Api.Primitives.flatten file |> List.ofSeq
        ...
        testCase "tryFind: existing path returns Some" <| fun () ->
            ...
            | Error es -> failtestf "parse failed: %A" es
            | Ok file ->
                match NEXUS.DesignTokens.Api.Primitives.tryFind ["color"; "brand"] file with
                | Some _ -> ()
        ...
        testCase "tryFind: missing path returns None" <| fun () ->
            ...
            | Error es -> failtestf "parse failed: %A" es
            | Ok file ->
                match NEXUS.DesignTokens.Api.Primitives.tryFind ["color"; "missing"] file with
                | None -> ()
        ...
        testCase "tryResolveAlias: follows curly-brace ref" <| fun () ->
            ...
            | Error es -> failtestf "parse failed: %A" es
            | Ok file ->
                match NEXUS.DesignTokens.Api.Primitives.tryResolveAlias (CurlyBrace ["color"; "primary"]) file with
                | Some t ->
                    match t.Value with
