...
module Color =
    module Action =
        let Default = "var(--color-action-default)"
        let Hover   = "var(--color-action-hover)"
    module Machine =
        module Washer =
...
module Spacing =
    module Touch =
        let Min = "var(--spacing-touch-min)"
```

Rules: PascalCase all segments; N-prefix for segments starting with a digit (`500` → `N500`);
hyphens become camel joins (`focus-ring` → `FocusRing`). F# keywords capitalise cleanly
(`default` → `Default`) so no backtick-escaping is needed in practice.

Binding type is `string` — values are `"var(--name)"` ready for use in Fun.Css property builders.
No runtime dependency on `FnTools.DesignTokens` in the generated file.

LaundryLog generated bindings: `tokens/Tokens.fs` (161 lines).
